...
export class CreateAccountComponent implements OnInit, OnDestroy {
  ...
  async signupFormSubmit() {
    try {
      ...
      this.packObject();
      this.authService
        .sendCreateAccountFormData(this.user)
        .pipe(takeUntil(this.unsubscribeAll))
        .subscribe((res) => {
          if (typeof res === 'string') {
            if (res === 'Invalid Office') {
              this.invalidMessage =
                'There was a problem with your registration. Please contact us at info@mediaframework.com or 201-801-5228 to complete your registration.';
            } else {
              this.invalidMessage = '';
  ...
  async verificationCodeSubmit() {
    try {
      this.spinnerService.show();
      this.authService
        .codeCheckWithEmail(this.user.email, this.codeForm.value.code)
        .pipe(takeUntil(this.unsubscribeAll))
        .subscribe((res) => {
          if (res == '') {
            this.attempts++;
            if (this.attempts < this.MAX_TRIES) {
              ...
              this.toastr.danger(
                'You have attempted to enter the code too many times. The code found in the email is no longer valid.'
              );
            }
          } else {
            this.authService
              .checkCreateAccountValidate(res)
              .pipe(takeUntil(this.unsubscribeAll))
              .subscribe((valid) => {
                if (valid) {
  ...
  async verificationCodeSendAgain() {
    try {
      ...
      this.invalidMessage = '';
      this.authService
        .sendCreateAccountCode(this.user.email)
        .pipe(takeUntil(this.unsubscribeAll))
        .subscribe((res) => {
          if (typeof res === 'string' && res == 'message sent') {
            this.attempts = 0;
            this.toastr.success(
              'Another code has been sent to ' +
                this.user.email +
                '. Check your spam folder. It may be hiding in there.'
            );
          }
        });
    } catch (e: any) {
  ...
  async recaptchaResolved(captchaResponse: string) {
    try {
      ...
      if (captchaResponse) {
        this.authService
          .recaptchaValidate(captchaResponse)
          .pipe(takeUntil(this.unsubscribeAll))
          .subscribe((res: any) => {
