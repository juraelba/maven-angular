...
type ComplexDataStructure = { options: SelectOption[] } & {
  [key: string]: boolean;
};
...
export class SearchService {
  ...
  createSearch(
    { columns, criteria }: SearchQuery,
    key: SearchKey
  ): Observable<CreateSearchResponse> {
  ...
  transformComplexData({
    options,
    ...rest
  }: ComplexDataStructure): TransformedSearchData {
  ...
  transformDiverseTargets(
    criteriaData: ComplexDataStructure
  ...
  transformMarketsData({
    options,
    market,
  }: MarketCriteria): TransformedSearchData {
    const { searchOptions } = this.transformOptions(options);

    return { searchOptions, columns: {}, criteriaKey: market };
  }

  transformMatchedToData({
    matchedTo,
    matched,
  }: MatchedToCriteria): TransformedSearchData {
  ...
  transformNonComsCriteriaData(value: boolean): TransformedSearchData {
    return {
      searchOptions: value,
      columns: {},
      criteriaKey: SearchFiedlsEnum.nonComms,
    };
  }

  transformCriteriaKey(
    key: SearchFiledsKey
  ): (criteriaOptions: SelectOption[]) => TransformedSearchData {
  ...
  transformCriteriasToSearchOptions(criterias: Criteries): SearchQuery {
    const trasformers: Transformers = {
      ...
      [SearchFiedlsEnum.metric]: this.booleanCriteriaToData.bind(
        this,
        SearchFiedlsEnum.metric
      ),
      [SearchFiedlsEnum.slogan]: this.booleanCriteriaToData.bind(
        this,
        SearchFiedlsEnum.slogan
      ),
      ...
      [SearchFiedlsEnum.tvnetworks]: this.transformCriteriaKey(
        SearchFiedlsEnum.networks
      ),
      [SearchFiedlsEnum.tvbands]: this.transformCriteriaKey(
        SearchFiedlsEnum.bands
      ),
      [SearchFiedlsEnum.radiobands]: this.transformCriteriaKey(
        SearchFiedlsEnum.bands
      ),
      [SearchFiedlsEnum.mavenId]: this.booleanCriteriaToData.bind(
        this,
        SearchFiedlsEnum.mavenId
      ),
      [SearchFiedlsEnum.addDirector]: this.booleanCriteriaToData.bind(
        this,
    ...
    return compose<[Criteries], [string, Criteries][], SearchQuery>(
      reduce<[string, Criteries], SearchQuery>(
        (acc, [key, value]) => {
  ...
  transformSearchResultToTableData(
    searchResult: SearchResponse,
    searchQuery: SearchQuery,
    searchKey: SearchKey
  ): Table {
  ...
  validateCell(
    filterOperator: FilterOperatorKey,
    filters: Filter[],
    cellValue: unknown
  ): boolean {
    ...
    return filters.length
      ? filters[method](({ textFilterType, value }) =>
          this.validate(textFilterType, value, cellValue)
        )
      : true;
  }

  filterDataBasedOnColumnAutoFilters(
    rows: Row[],
    ...
    return rows.filter((row) => {
      ...
      const isValidRowData = pair.every(([key, cellValue]) => {
        ...
        const cellANDFilters = this.getFilterByOperator(
          cellFilters,
          FilterOperatorEnum.AND
        );
        const cellORFilters = this.getFilterByOperator(
          cellFilters,
          FilterOperatorEnum.OR
        );

        const isANDValid = this.validateCell(
          FilterOperatorEnum.AND,
          cellANDFilters,
          cellValue
        );
        const isORValid = this.validateCell(
          FilterOperatorEnum.OR,
          cellORFilters,
          cellValue
        );
  ...
  mapFilters(filters: ColumnAutoFilterData): { [key: string]: Filter[] } {
    return compose<
      ...
      { [key: string]: Filter[] }
    >(
      reduce<[string, ColumnAutoFilterValue], { [key: string]: Filter[] }>(
        (acc, [key, value]) => {
